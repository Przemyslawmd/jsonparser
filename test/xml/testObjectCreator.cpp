...
protected:
    std::unique_ptr<ObjectNode> createObjects(const std::string& path, const std::string& file)
    {
        ...
        auto objCreator = std::make_unique<ObjectCreator>();
...
TEST_F(TestObjectCreator, test_1)
{
    ...
    ASSERT_TRUE(node->find(0) != node->end());
    auto* nodeValue = std::get_if<ObjectNode>(&node->at(0).value);
    ASSERT_TRUE(nodeValue != nullptr);

    auto* nodeValue_1 = std::get_if<std::string>(&nodeValue->at(0).value);
